...
if (initialState.currentUser) {
    ...
    initialState.saveItems = JSON.parse(localStorage.getItem(userSaveitemKey)) || [];
};
    ...
    reducers: {
        setCurrentUser: (state, action) => {
            ...
            if (action.payload) {
                ...
                state.saveItems = JSON.parse(localStorage.getItem(userSaveitemKey)) || [];
            } else {
                state.saveItems = [];
            }
        },
        ...
        addSaveforLater: (state, action) => {
            ...
            const existingItem = state.saveItems.find((item) => item.id === action.payload.id);
        ...
        removeForSaveLater: (state, action) => {
            ...
            const userSaveitemKey = `saveForLaterItem_${userId}`;
            state.saveItems = state.saveItems.filter((item) => item.id !== action.payload);
            localStorage.setItem(userSaveitemKey, JSON.stringify(state.saveItems));
        },
...
export const {addSaveforLater, removeForSaveLater, clearSaveForLater, setCurrentUser} = saveForLaterSlice.actions;
